...
PROFILES_PATH = os.path.join(
    os.path.dirname(__file__), "database", "profiles.json"
)
...
async def get_profile_by_name(
    name: str = Path(..., description="Name of the profile to retrieve")
):
    """
    ...
    try:
        ...
        if not profile:
            raise HTTPException(
                status_code=404,
                detail=f"Profile with name '{decoded_name}' not found",
            )
...
async def get_meeting_summary():
    """
    Generate and retrieve a summary of the current conversation
    """
    try:
        summary = await summary_service.generate_meeting_summary()
        return summary
    except Exception as e:
    ...
    try:
        ...
        while True:
            ...
            if event_type == "connected":
                ...
                if not transcriber:
                    logger.error(
                        "Received media before transcriber was connected"
                    )
